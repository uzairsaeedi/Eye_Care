...
public class ProgressUtils {
    ...
    private static final String KEY_PER_EX_PREFIX = "completed_"; // optional per-ex attempts

    /**
     * Mark that the user completed a test for given exerciseType.
     * If you want to count only first-time completion per exercise toward the overall progress,
     * use markTestCompletedUnique(); otherwise use this to count every completion.
     */
    public static void markTestCompleted(Context ctx, String exerciseType) {
        ...
        // increment per-exercise attempts (optional)
    ...
    /**
     * Alternative: mark first-time completion only once (won't increment overall on repeats)
     */
    public static void markTestCompletedUnique(Context ctx, String exerciseType) {
        ...
        // still increment per-ex attempts if you want
        String perKey = KEY_PER_EX_PREFIX + exerciseType;
        int perCount = prefs.getInt(perKey, 0);
        prefs.edit().putInt(perKey, perCount + 1).apply();
    }
     ...
     * total should be > 0 (e.g. QuestionBank.getTotalExercises())
     */
